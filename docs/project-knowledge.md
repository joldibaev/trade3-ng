...
- **`ui/`**: Переиспользуемые примитивные UI компоненты.
  ...
  - **`ui-listbox`**: Компонент для выбора элементов из списка.
    - Построен на базе `@angular/aria/listbox`.
    - Использует универсальную стилевую абстракцию `.ui-list` и `.ui-list-item`, (определены в `src/styles/components/_list.css`).
    - Поддерживает шаблонизацию элементов через `<ng-template>`.
...
- **Кастомные утилиты**:
  ...
  - `h1`: Типографика для заголовков первого уровня.
...
- **Поиск с Debounce**: В диалогах для поисковых полей используется паттерн с `toObservable`:
  - Создается сигнал состояния формы.
  - В конструкторе (в injection context) выполняется `toObservable` -> `debounceTime` -> `subscribe` для обновления отдельного сигнала запроса API. Это предотвращает лишние вызовы при вводе.
...
3. **Выбрано**: При выборе элемента автокомплит заменяется на блок с названием выбранной сущности и кнопкой "Сброс" (крестик).
   - Это предотвращает случайное изменение и четко показывает текущий выбор.
   - Реализуется через переключение `@if (selectedName) { ... } @else { ... }`.

### Двухэтапное создание элементов

Для сложных сущностей (например, позиция покупки) применяется разделение на два диалога:

1. **Диалог выбора (Selection Dialog)**: Отвечает только за поиск и выбор основной сущности (например, `ProductSelectDialog`). Содержит автокомплит/поиск и список результатов.
2. **Диалог деталей (Details Dialog)**: Открывается после успешного выбора, принимает ID выбранной сущности. Содержит форму ввода количества, цены и других параметров.

- Это упрощает UX, разделяя задачи поиска и ввода данных.
